...
description: >-
  ...
  the official template and this repo’s conventions.
---
...
- Team Marketplace import of this GitHub repo

## Steps

1. **If the repo has a validator, run it.** The official template ships `node scripts/validate-template.mjs`. This Lloyd Sity marketplace does **not** include that script — use the manual list below.
2. Manual checklist (from the [official template README](https://github.com/cursor/plugin-template) plus this repo):
   - [ ] Each plugin has a valid `.cursor-plugin/plugin.json`
   - [ ] Plugin names are unique, lowercase, kebab-case
   - [ ] Marketplace `plugins[].source` maps to a real folder **from the repo root** (here: `"clarity-gate"`, `"build-origin"`, `"grok-plugin-maker"` — not `plugins/…`)
   ...
   - [ ] `README.md` and MIT `LICENSE` are present; author is **Lloyd Sity**
   - [ ] No `starter-simple` / `starter-advanced` / `Your Org` placeholders
   - [ ] No invented emails or secrets
3. Confirm the root `README.md` lists every plugin in `marketplace.json`.
4. **Publish path for this marketplace:**
   - Public: [cursor.com/marketplace/publish](https://cursor.com/marketplace/publish) — submit the GitHub repo after it is public
   - Team: Dashboard → Plugins → Import this GitHub repo
