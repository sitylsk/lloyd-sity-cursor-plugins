...
description: >-
  Pre-submission checklist for a Cursor marketplace plugin (Simple or
  Advanced). Use when the user is ready to publish, import to a team, or
  wants a validation pass against the official template.
---
...
2. Manual checklist (official template README + marketplace conventions):
   - [ ] Each plugin has a valid `.cursor-plugin/plugin.json`
   - [ ] Plugin names are unique, lowercase, kebab-case
   - [ ] Marketplace `plugins[].source` maps to a real folder **from the repo root** (in this marketplace repo: `"clarity-gate"`, `"build-origin"`, `"plugin-maker"` — not `plugins/…`; template-style repos use `./plugins/<name>`)
...
3. **Shape-specific (do not fail Simple for missing advanced files):**
   - **Simple:** only `rules/` + `skills/` expected. Missing `hooks/hooks.json` / `mcp.json` is OK (validators may *warn*).
   - **Advanced:** each component you shipped must be real — agent/command frontmatter; `hooks/hooks.json` commands resolve; `mcp.json` has no secrets; every `${VAR}` is in `plugin.json` `variables`. Sample `postgres` / `format-code.sh` leftovers count as fail if they are not the product.
4. Confirm the root `README.md` lists every plugin in `marketplace.json` (multi-plugin repos).
5. **Publish path:**
...
6. After submit, keep a short note of what was checked (pass/fail). Fix errors before asking the user to click publish.
...
- Do not require hooks or MCP for Simple plugins.
